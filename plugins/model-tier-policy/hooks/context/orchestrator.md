...
- Dispatch with pinned models: "{executor}" (Opus) implements, "{senior}" (Fable) for entangled work, "{scout}" (Opus,
  ...
  ten-word dispatch, not a git session. Cap every return — and every brief: constants live in the operating-rules file
  (point, never restate), bulk content goes by file path. Spell each `subagent_type` exactly as written here — that is
