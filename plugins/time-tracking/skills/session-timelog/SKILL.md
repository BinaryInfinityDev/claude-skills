...
2. **Extract timestamps only** with `jq`: `select(type=="object" and .timestamp!=null) | {timestamp, type, sessionId}` —
   ...
   builtin.
3. **Scan the extract clean** — zero matches required for `@`, `/home/`, `/root/`, `proxy`, and `trustStore`. The script
   aborts on a hit; investigate before retrying. Never commit a dirty extract.
