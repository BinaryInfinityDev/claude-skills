...
A compaction summary is memory of the least reliable kind: it keeps outcomes and drops provenance, and a missing actor
reads back as the reader. After a compaction, every remembered actor, approval, and precedent is unverified — the
tracker's `last` and `auth` columns, and one call per claim, stand in for it; nothing irreversible is performed or
dispatched on the summary's word.
